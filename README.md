...
Note that this repo provides the SOTA performance on distributed full-batch (full-graph) GNN training even without the schemes of GraNNDis from our own optimizations using NCCL.
Our implementations are mainly based on the original code of PipeGCN.
For details, please refer to our PACT'24 paper.

## Dear AE Reviewers
We have prepared authors' remote machine for the artifact evaluation process.
The cluster have two servers and each server has four NVIDIA RTX A6000 GPUs.
Internal server GPUs are connected via NVLink Bridge and servers are connected via 10GbE.
**When the reviewers request the remote server using HotCRP comments, the authors will provide detailed guideline for accessing the authors' remote machine as soon as possible.**
...
- Prerequisite
  ...
  - Anaconda Setting
  - NFS environment with more than two servers, each server has multiple-GPUs.
...
- For the artifact evaluation, we need enough system memory (e.g., 256GB).
...
For the artifact evaluation, we will use three sample datasets (i.e., Arxiv, Reddit, Products), which are widely used and easily accessible datasets.
To download the datasets and test the created environment, we provide a test script (`Codes/brief_masking_test.sh`).
...
If the tests are successefully conducted, the logs will be saved in `Codes/masking_test_logs/`.
...
As distributed training procedure could be not familiar with some users, we provide simple distributed experiment launchers at `AE/*.py`.
Before reproducing, users are required to change the configuration fields in the launchers in `AE/*.py`.
```
...
```
**For the artifact evaluation process, we already changed the configuration fields for authors' remote machine.**

After modification, just run the following commands will show the artifact evaluation results.
```
...
Followings are the example results when running the above procedure in the authors' remote machine.

### AE1. Throughput Results (Flexible Preloading (FLX), Cooperative Batching (CoB), and Expansion-Aware Sampling (EAS))
From the results, we can figure out that optimized full-batch training baseline (Opt_FB) suffers from communication overhead, while FLX/CoB address such an issue through server-wise preloading.
...
As EAS only targets to sample server boundary vertices which contributes to acceleration, it successfully achieves the comparable accuracy to the original full-batch training.
```
+--------------------------------------+
...
--sampler sage # use node-wise sampling
--epoch-iter 1 # #iters/epoch, use a larger vlue if you need finer-grained mini-batch
--use-mask # use 1-hop graph masking to support intact full-batch/mini-batch training algorithm
```
...
--sampler sage # use node-wise samplingg
--use-mask # use 1-hop graph masking to express dependency
```

### 3. Distributed Launch
We provide a simple distributed experiment runner interface for users who are not familiar with distributed launch of training.
...
For other codes, license also under the MIT license.

## MISC

For further breakdown internal/external communication time, users can utilize `--check-intra-only` option. It will ignore the external server communication, so users can figure out the internal server communication time only.
